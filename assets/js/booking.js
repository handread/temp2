/* ===========================================================
   预约系统 · step wizard (演示原型,数据全部为本地模拟)
   =========================================================== */
...
(() => {
  ...
  /* ---------------- step 1: service ---------------- */
  const cats = ["全部", ...Array.from(new Set(YXDATA.SERVICES.map(s=>s.cat)))];
  let activeCat = "全部";
  ...
  function renderSvcList(){
    const list = activeCat === "全部" ? YXDATA.SERVICES : YXDATA.SERVICES.filter(s=>s.cat===activeCat);
  ...
  function renderSummary(targetSel){
    ...
    $(targetSel).innerHTML = `
      <div class="row"><span class="muted">项目</span><b>${svc.name}</b></div>
      <div class="row"><span class="muted">顾问</span><b>${staff.name}</b></div>
      <div class="row"><span class="muted">时间</span><b>${y}-${m}-${d} ${state.time}</b></div>
  ...
  function init(){
    ...
    goStep(1);
    if(state.svc) YX.toast("已根据 AI 分析建议为您预选项目");
  }
