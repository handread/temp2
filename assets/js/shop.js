...
(() => {
  ...
  const cats = ["全部", ...Array.from(new Set(YXDATA.PRODUCTS.map(p=>p.cat)))];
  let activeCat = "全部";
  let selectedPay = "微信支付";
  let modalQty = 1;
  ...
  function renderGrid(){
    const list = activeCat==="全部" ? YXDATA.PRODUCTS : YXDATA.PRODUCTS.filter(p=>p.cat===activeCat);
    $("#productGrid").innerHTML = list.map(p => `
      <div class="prod-card" data-id="${p.id}">
        ...
        <div class="prod-body">
          ...
          <h4>${p.name}</h4>
          <div class="rating">★ ${p.rating} · 已售 ${p.sold}</div>
          <div class="price-row">
  ...
  function openProduct(id){
    ...
    $("#prodModalBody").innerHTML = `
      ...
      <div class="rating" style="margin-top:.4rem">★ ${p.rating} · 已售 ${p.sold}</div>
      ...
      <button class="btn btn-primary btn-block" id="modalAddBtn">加入购物车</button>
    `;
    ...
    $("#modalAddBtn").addEventListener("click", () => {
      ...
      YX.toast(`已加入购物车:${p.name} × ${modalQty}`);
      closeModal("prod");
  ...
  function renderCart(){
    ...
    updateCartCountLabel();
    if(!cart.length){
      $("#cartBody").innerHTML = `<div class="empty-cart"><div class="ic" style="font-size:2rem">🛍️</div>购物车还是空的,去挑选心仪的商品吧</div>`;
        ...
        <div class="cart-row">
          ...
          <div class="info">
            ...
            <div class="qty-ctrl">
              ...
              <button class="cart-remove" data-rm="${item.id}">删除</button>
            </div>
          </div>
        </div>
      `;
  ...
  function openCheckout(){
    const cart = YX.getCart();
    if(!cart.length){ YX.toast("购物车是空的"); return; }
    $("#checkoutForm").style.display = "block";
    $("#checkoutSuccess").style.display = "none";
    $("#checkoutTitle").textContent = "确认订单";
    const total = cart.reduce((s,i)=>s+i.price*i.qty,0);
    ...
    `).join("") + `<div class="row" style="display:flex;justify-content:space-between;font-size:.9rem;padding-top:.6rem;margin-top:.4rem;border-top:1px dashed var(--line)"><span>合计</span><b style="color:var(--magenta)">${YX.fmtCNY(total)}</b></div>`;
    closeModal("cart");
    openModal("checkout");
  }
  ...
  function confirmPay(){
    ...
    $("#checkoutTitle").textContent = "订单完成";
    YX.clearCart();
    renderCart();
    YX.toast("模拟支付成功,订单已生成");
  }
