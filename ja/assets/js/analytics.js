...
(() => {
  ...
  function renderKpis(){
    $("#kpiGrid").innerHTML = D.kpis.map(k => `
      <div class="stat-tile">
        ...
        <div class="delta ${k.up?'up':'down'}">${k.up?'▲':'▼'} ${k.delta} <span class="muted" style="margin-left:.2em">较上月</span></div>
      </div>
    `).join("");
  }
  ...
  function renderTier(){
    YXChart.donutChart($("#tierChart"), { data: D.memberTier, size:200, thickness:26, centerLabel:"会员总数", yFormat:v=>v.toLocaleString() });
  ...
  function renderChannel(){
    ...
    YXChart.donutChart($("#channelChart"), { data: withColors, size:200, thickness:26, centerLabel:"占比", yFormat:v=>v+"%" });
  ...
  function buildTableData(chartId){
    switch(chartId){
      case "revenueChart":
        return {
          headers: ["月份", ...D.revenueTrend.series.map(s=>s.name)],
          rows: D.revenueTrend.labels.map((l,i)=>[l, ...D.revenueTrend.series.map(s=>s.data[i])]),
        };
      case "serviceChart":
        return { headers:["施術","预约人次"], rows: D.servicePopularity.map(d=>[d.name,d.value]) };
      case "tierChart":
        return { headers:["等级","人数"], rows: D.memberTier.map(d=>[d.name,d.value]) };
      case "channelChart":
        return { headers:["渠道","占比 (%)"], rows: D.channel.map(d=>[d.name,d.value]) };
      case "skinChart":
        return { headers:["问题类型","人次"], rows: D.skinConcern.map(d=>[d.name,d.value]) };
    }
    return { headers:[], rows:[] };
  }
