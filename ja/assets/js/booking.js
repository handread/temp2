/* ===========================================================
   预约系统 · step wizard (演示原型,数据すべて为本地模拟)
   =========================================================== */
...
(() => {
  ...
  /* ---------------- step 1: service ---------------- */
  const cats = ["すべて", ...Array.from(new Set(YXDATA.SERVICES.map(s=>s.cat)))];
  let activeCat = "すべて";
  ...
  function renderSvcList(){
    const list = activeCat === "すべて" ? YXDATA.SERVICES : YXDATA.SERVICES.filter(s=>s.cat===activeCat);
    $("#svcList").innerHTML = list.map(s => `
      <div class="svc-card ${state.svc===s.id?'sel':''}" data-id="${s.id}">
        <div class="row">
          <h4>${s.name}</h4>
          <div class="price">${s.price===0?'免费':YX.fmtCNY(s.price)}</div>
        </div>
        <p>${s.desc}</p>
        <div class="meta">${s.cat} · 约 ${s.duration} 分钟</div>
      </div>
    `).join("");
  ...
  function renderStaffList(){
    ...
    $("#staffList").innerHTML = list.map(s => `
      <div class="staff-card ${state.staff===s.id?'sel':''}" data-id="${s.id}">
        ...
        <div class="role" style="margin-top:.3rem">${s.years?`从业 ${s.years} 年`:'AI 实时在线'}</div>
      </div>
    `).join("");
  ...
  function renderSummary(targetSel){
    ...
    $(targetSel).innerHTML = `
      <div class="row"><span class="muted">施術</span><b>${svc.name}</b></div>
      <div class="row"><span class="muted">担当</span><b>${staff.name}</b></div>
      <div class="row"><span class="muted">日時</span><b>${y}-${m}-${d} ${state.time}</b></div>
      <div class="row"><span class="muted">时长</span><b>约 ${svc.duration} 分钟</b></div>
      <div class="row"><span class="muted">费用</span><b style="color:var(--magenta)">${svc.price===0?'免费':YX.fmtCNY(svc.price)}</b></div>
    `;
  }
  ...
  function submitBooking(){
    ...
    const booking = {
      ...
      status: "待到店", createdAt: new Date().toISOString(),
    };
    const list = getBookings();
    list.unshift(booking);
    localStorage.setItem(BOOKINGS_KEY, JSON.stringify(list));
    $("#confCode").textContent = "预约编号 " + code;
  ...
  function renderMyBookings(){
    ...
    if(!own.length && !history.length){
      mount.innerHTML = `<div class="empty"><div class="ic">🗓️</div>暂无预约记录</div>`;
      return;
    }
    const ownHtml = own.map(b => `
      <div class="card mybooking-card">
        <div>
          <div class="flex gap-1 center-y"><span class="tag">${b.status}</span><span class="muted" style="font-size:.78rem">编号 ${b.code}</span></div>
          <h4 style="margin-top:.5rem">${b.svcName}</h4>
          <p class="muted" style="font-size:.84rem;margin-top:.3rem">${b.date} ${b.time} · ${b.staffName} · ${b.price===0?'免费':YX.fmtCNY(b.price)}</p>
        </div>
        <button class="btn btn-ghost btn-sm" data-cancel="${b.code}">取消预约</button>
      </div>
    `).join("");
  ...
  function init(){
    ...
    goStep(1);
    if(state.svc) YX.toast("已根据 AI 分析建议为您预选施術");
  }
